...
function calculateRANSAC(samples: Coordinate[]): { result: Coordinate; usedIndices: number[] } {
  ...
  return calculateWeightedLSE(bestInliers.map(idx => samples[idx]));
    ...
    <h3>2.4.8. DBSCAN (Density-Based Spatial Clustering) + WLS Birleşik Jeodezik Filtresi</h3>
    <p>Akıllı telefonların entegre GNSS entegreleri (özellikle tek frekanslı L1 alıcılar), kentsel kanyonlarda binalarca saptırılan veya ağaç kaplı alanlarda saçılan uydulardan dolayı son derece yüksek gürültülü koordinat çıktıları üretir. Yapılan saha testlerinde L1 alıcılarda en az 1.0 m, ortalama ise 5.0 m mertebesinde düzlemsel saçılmalar ve geçici sinyal sekmesi (multipath jumps) kaynaklı "sahte kümelenmeler" gözlenmektedir. RANSAC tekil ayıklama modeli gürültülü ve çok kutuplu dağılımlarda tek bir modele yakınsayarak diğer noktaları tamamen gürültü sayarken; KMeans ise gürültü noktalarını da kısıtlı kümelere zorla dahil ederek konumsal merkezi saptırır.</p>

    <p><b>DBSCAN (Density-Based Spatial Clustering of Applications with Noise)</b>, jeodezik veri süzme işleminde hiçbir fonksiyonel kısıt (örneğin küme sayısı k) varsaymaksızın ham konum gözlemlerini tamamen mekansal yoğunluk profillerine göre gruplar. Bu sayede, çoklu yol yansıması içermeyen ve uydulardan doğrudan kilidi koruyan temiz epoklar "Ana Konum Havuzu (Core Cluster)" olarak tescillenirken, 1-5 metre arasında rasgele saçılan veya sekme yapan tüm aykırı milisaniyelik gürültü noktaları "Noise (Safi Gürültü)" etiketini alarak WLS aşamasına geçirilmeden tamamen veri setinden elenir.</p>
    ...
    <div class="case-container" style="background-color: #f0fdf4; border-left: 4px solid #16a34a; padding: 12px; margin-bottom: 20px; font-size: 10pt;">
      <p class="bold" style="color: #166534; margin-bottom: 6px;">DBSCAN + WLS Uyarlamalı Kombinasyon Protokolü</p>
      <p class="no-indent" style="margin-bottom: 5px;"><span class="bold">1. Adaptif Epsilon Komşuluk Yarıçapı (&epsilon;):</span> Gözlem serisinin düzlemsel koordinatlarının standart sapması (&sigma;<sub>spatial</sub>) hesaplandıktan sonra, akıllı telefonun anlık gürültü dağılımına uyum sağlayan dinamik bir komşuluk dairesel yarıçapı tayin edilir:
        <br/><span class="bold" style="display: block; text-align: center; margin: 8px 0; font-family: monospace;">&epsilon; = Max(0.75m, Min(4.50m, &sigma;<sub>spatial</sub> &times; 0.80))</span>
        Bu yaklaşım sayesinde, temiz veri gruplarında arama dairesi daralarak yüksek çözünürlükte süzme yaparken; L1 alıcılardaki yüksek saçınımlarda genişleyerek (maksimum 4.5m) sistemin parçalanmasını önler.
      </p>
      <p class="no-indent" style="margin-bottom: 5px;"><span class="bold">2. Dinamik Minimum Nokta Sayısı (MinPts):</span> Komşu dairesi içerisinde en az bulunması gereken gözlem sayısı (epok adedi), toplam epok sayısının %15'i olacak şekilde dinamik olarak sınırlandırılır (en az 3, en fazla 10):
        <br/><span class="bold" style="display: block; text-align: center; margin: 8px 0; font-family: monospace;">MinPts = Min(10, Max(3, &lfloor;N &times; 0.15&rfloor;))</span>
      </p>
      <p class="no-indent" style="margin-bottom: 5px;"><span class="bold">3. Şampiyon Yoğunluk Grubu Kararı:</span> DBSCAN tarafından bölümlenen yoğunluk gruplarından en fazla elemana sahip ana yoğunluk kümesi seçilir. Eğer eleman sayıları eşit ise koordinat saçılım varyansı en küçük olan grup "Şampiyon Küme" ilan edilir.</p>
      <p class="no-indent" style="margin-bottom: 5px;"><span class="bold">4. Ağırlıklı Dengeleme (WLS):</span> Gürültüden (noise) tamamen arındırılmış şampiyon küme koordinatları donanım hassasiyet ters karesine göre (1 / accuracy<sup>2</sup>) WLS dairesine sokularak hassas milimetri seviyesinde dE ve dN konum çözümü üretilir.</p>
    </div>

    <h3>2.4.9. Sinyal Güvenilirlik Analizi ve Veri Saçılım Metodolojisi</h3>
