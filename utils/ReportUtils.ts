...
export function analyzeSignalReliability(samples: Coordinate[]): SignalAnalysis {
  ...
  return {
    ...
    <p>Bu karar matrisinin en kritik çıktısı, nihai filtrelenmiş verinin koordinat özniteliğindeki doğruluk (accuracy) değerinin belirlenmesidir: <b>Doğruluk = Maksimum (Maksimum Saçılım Genişliği, Ortalama GNSS Alıcısı Tahmini Konum Hatası - UERE Tabanlı / Accuracy Radius)</b>. Bu formülasyon sayesinde, cihaz yapay olarak çok yüksek bir hassasiyet bildirse bile (yani tahmini konum hata çemberi yarıçapını dar gösterse bile, Örn: 2m), veriler arazide çevre parazitlerinden dolayı 6 metrelik bir alana saçılıyorsa, sistem güvenli tarafta kalmak üzere kullanıcıya gerçekçi yatay hassasiyet yarıçapını 6 metre olarak ilan eder.</p>
