...
const N = (1 - u) * (1 - v) * n00 
    ...
    <p>En Küçük Kareler (LSE) prensibine göre, jeodezik bir ölçünün ağırlığı ($p$), o ölçünün karesel ortalama hatasının (veya standart sapmasının) karesiyle ters orantılıdır ($p = 1/\sigma^2$). Akıllı konum sensörlerinde tarayıcı düzeyinde elde edilen Geolocation API hassasiyet dairesi yarıçapı ($accuracy$), doğrudan ham standart sapmayı ($\sigma$) değil; pratik kestirim dünyasında %95 güven aralığına karşılık gelen bir dairesel hata olasılığını (Circular Error Probable - CEP veya yaklaşık 2-sigma) temsil eder. CEP değeri standart sapma ile doğrusal bağıntılı olduğundan, bu dairesel hata yarıçaplarının karesiyle ters orantılı ağırlıkların atanması ($p_i = 1/\text{accuracy}_i^2$), teorik jeodezik ağırlıklandırma modeliyle tam bir stokastik uyum sergiler.</p>
...
export function calculateMidRange(samples: Coordinate[]): { result: Coordinate; usedIndices: number[] } {
  ...
  return {
    ...
    <ul>
      <li><span class="bold">Ortalama GNSS Alıcısı Tahmini Konum Hatası ($o_{avg}$):</span> Alıcı cihazın Geolocation API aracılığıyla saniye bazında bağımsız olarak bildirdiği, UERE ve uydu geometrisi (DOP) bütününe dayalı yatay hassasiyet çemberi (accuracy radius) değerlerinin aritmetik ortalamasıdır:
...
export function analyzeSignalReliability(samples: Coordinate[]): SignalAnalysis {
  ...
  return {
    ...
    <ul>
      <li><span class="bold">1. Birleşik İstatistiki Doğruluk Modeli (Combined Statistical Uncertainty):</span> Süzgeç algoritmaları ve ağırlıklı merkez denge hesaplamalarında ($N > 1$ için), konumsal standart sapmanın ($\sigma_{spatial}$) ortalama standart hatası ($SEM$) ile her epoka düşen tahmini donanımsal konum hatası karesel (kuadratik) olarak birleştirilir:
        <br/><span class="bold" style="display: block; text-align: center; margin: 10px 0;">$$\text{Standard Error (SE)} = \frac{\sigma_{spatial}}{\sqrt{N}} \quad \Rightarrow \quad \text{finalAccuracy} = \sqrt{\text{SE}^2 + \left(\frac{o_{avg}}{\sqrt{N}}\right)^2}$$</span>
      ...
      <li><span class="bold">2. Nihai Muhafazakar Karar Limit Doğruluğu (Conservative Boundary Accuracy):</span> İstatistiki ağırlıklı dengelemenin getireceği iyimser tahminlerin, arazideki ani fiziki ve beklenmedik sinyal sıçramalarını maskelemesini önlemek amacıyla, deterministik en kötü durum (worst-case) tavan limiti uygulanır:
        <br/><span class="bold" style="display: block; text-align: center; margin: 10px 0;">$$\text{Conservative Accuracy} = \max(d_{max}, o_{avg})$$</span>
      </li>
    </ul>

    <p class="no-indent">Bu çift-kademeli hibrit bütünleştirme sayesinde, cihaz yapay olarak çok yüksek bir hassasiyet bildirse bile (yani tahmini konum hata çemberi yarıçapını dar gösterse bile, Örn: $2\text{m}$), veriler arazide çoklu yansımadan dolayı $6\text{m}$'lik bir alana saçılıyorsa, sistem güvenli tarafta kalmak üzere kullanıcıya gerçekçi yatay hassasiyet yarıçapını $6\text{m}$ olarak ilan eder. Aynı şekilde, gürültüden arındırılmış veri gruplarında da en hassas milimetrik ve santimetrik konumlandırma çözümleri istatistiksel standart hata formülüyle hassas bir biçimde korunur.</p>
