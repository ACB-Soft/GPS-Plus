      ...
      <div className="soft-card p-5 md:p-6 border-blue-100 space-y-5 md:space-y-6 text-center animate-in relative overflow-hidden bg-white max-w-sm mx-auto">
        <div className="space-y-2 md:space-y-3">
          ...
          <div className="space-y-1">
            <p className="text-[14px] md:text-[16px] font-black text-slate-900 uppercase tracking-[0.3em] opacity-90 leading-none">{location.folderName}</p>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 md:gap-4">
            ...
            <div className={`p-4 md:p-5 rounded-2xl md:rounded-3xl border text-left transition-colors ${
              location.accuracy <= 10 ? 'bg-emerald-50/80 border-emerald-100' : 
              location.accuracy <= 20 ? 'bg-amber-50/80 border-amber-100' : 'bg-rose-50/80 border-rose-100'
            }`}>
      ...
      {showMap && (
        <div className="fixed inset-0 z-[9999] bg-black flex flex-col animate-in fade-in">
          ...
          <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-[10000] w-full max-w-xs px-6">
            <div className="bg-white/90 backdrop-blur-md p-5 rounded-2xl shadow-2xl border border-white/20 flex items-center justify-between gap-4">
